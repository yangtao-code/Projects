...
Lcd::Lcd()
{
    ...
    plcd = (unsigned int *)mmap(NULL, 800 * 480 * 4, PROT_READ | PROT_WRITE, MAP_SHARED, lcd_fd, 0);
    if (plcd == MAP_FAILED)
    {
        perror("mmap");
        exit(1);
...
void Lcd::display_Point(int x, int y, unsigned int color)
{
    if (x >= 0 && x < 800 && y >= 0 && y < 480)
        *(plcd + 800 * y + x) = color;
}

void Lcd::display_Square(int x0, int y0, unsigned int l, unsigned int w, unsigned int color)
{
...
void Lcd::display_Circle(int x0, int y0, int r, unsigned int color)
{
...
int Lcd::open_bmp(const char *bmp_name, int *width, int *hight, short *depth)
{
    // 1.打开bmp图片
...
int Lcd::display_bmp(int x0, int y0, const char *bmp_name)
{
    int width, hight;
    short depth;
    ...
    for (int y = 0; y < abs(hight); y++)
    {
        for (int x = 0; x < abs(width); x++)
        {
            ...
            display_Point(width > 0 ? x + x0 : abs(width) - x - 1 + x0,
...
int Lcd::display_jpg(int x0, int y0, const char *jpg_name)
{
