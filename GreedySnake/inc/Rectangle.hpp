...
namespace rectangle
{
    ...
    public:
         ...
         * @param y0 y轴坐标
         */
        void draw(screen::Screen &s, const int &x0, const int &y0)
        {
    ...
    Rectangle::Rectangle(const uint32_t &_w, const uint32_t &_h,
                         const uint32_t &_color)
        : w(_w), h(_h), fill_color(_color)
    {
    }

    void Rectangle::setcolor(const uint32_t &_color)
    {
